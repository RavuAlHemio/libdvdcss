/*****************************************************************************
 * libdvdcpxm.c: CPxM decryption library
 *****************************************************************************
 * Copyright (C) 2012
 * $Id$
 *
...
static uint64_t c2_enc( uint64_t code, uint64_t key )
{
    uint32_t L, R, t;
...
static uint64_t c2_dec( uint64_t code, uint64_t key )
{
    uint32_t L, R, t;
    uint32_t ktmpa, ktmpb, ktmpc, ktmpd;
    uint32_t sk[ 10 ];
...
static uint64_t c2_g( uint64_t code, uint64_t key )
{
...
static void c2_dcbc( void *p_buffer, uint64_t key, int length )
{
    uint32_t L, R, t;
    uint32_t ktmpa, ktmpb, ktmpc, ktmpd;
    uint32_t sk[ 10 ];
...
static uint8_t *cppm_get_mkb( const char *psz_mkb )
{
    FILE    *f_mkb;
    uint8_t *p_mkb = NULL;
...
static uint8_t *cprm_get_mkb( void )
{
...
static int cppm_set_id_album( void )
{
...
static int cprm_set_id_media( void )
{
...
static int process_mkb( uint8_t *p_mkb, device_key_t *p_dev_keys, int nr_dev_keys, uint64_t *p_media_key )
{
...
static int vr_get_k_te( char *psz_vr_mangr )
{
    FILE    *f_vr_m;
...
static int dvdcpxm_init( char *psz_target )
{
    char psz_dvddev[ 8 ];
...
static int mpeg2_check_pes_scrambling_control( uint8_t *p_block )
{
...
static void mpeg2_reset_pes_scrambling_control( uint8_t *p_block )
{
    if ( *( uint32_t* )p_block == 0xba010000 )
    {
...
static void mpeg2_reset_cci( uint8_t *p_block )
{
...
static int cppm_decrypt_block( uint8_t *p_buffer, int flags )
{
...
static int cprm_decrypt_block( uint8_t *p_buffer, int flags )
{
    uint64_t d_kc_i, k_au, k_i, k_c;
    int encrypted;
...
static int dvdcpxm_decrypt( void *p_buffer, int nr_blocks, int flags )
{
    switch ( media_type )
    {
    case COPYRIGHT_PROTECTION_CPPM:
